...
public class MyController extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) {
        ...
        router.get("/api/digFactorial").handler(this::digFactorial);
    ...
    private void digFactorial(RoutingContext routingContext) {
        HttpServerResponse response = routingContext.response();
        HttpServerRequest request = routingContext.request();
	
	String numero = request.getParam("numA");
 	
	long x = 0;
        long digits = -1;
        try {
             x = Long.parseLong(numero);
            digits = facDigits(x);
        } catch (Exception e) {
            digits = -1;
	}

        String jsonResponse = procesaFacto(numero, digits);
    ...
    private String procesaFacto(String numero, long digits) {
        Map<Object, Object> info = new HashMap<>();
        info.put("servidor", pba);
        info.put("Entrada", numero);
	info.put("digitos", digits);
